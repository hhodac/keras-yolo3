...
class YOLOv3:
    ...
    def __init__(self, input_shape=(416,416)):
        self.model = load_model('../yolov3_model.h5')
        self.anchors = [[116,90, 156,198, 373,326], [30,61, 62,45, 59,119], [10,13, 16,30, 33,23]]
        self.input_shape = input_shape # (height, width) order
        ...
        print("Model initialization completes.")
    ...
    def set_class_threshold(self, threshold):
        self.class_threshold = threshold

    def predict_image(self, tensor_image):
        """img and tensor_image must have the same size (width & height)
        """
        yhat = self.model.predict(tensor_image)
        return yhat

    def decode_prediction(self, yhat, orig_img_shape):
        image_w, image_h = orig_img_shape
        boxes = list()
...
def load_image(url, return_tensor: bool=False):
    """
    :url: coco image's url
    :shape: resize to (h, w)
...
# def preprocess_image(img, crop_area, resize_shape):
#     # crop image
#     if crop_area is not None:
#         img = img.crop((crop_area['x1'], crop_area['y1'], crop_area['x2'], crop_area['y2']))
#     # resize image
#     img_rsz = img.resize(resize_shape,resample=Image.BICUBIC)
    

def get_yolov3_catLabel():
    """Returns yolov3's category as dictionary type.
    """
...
def show_result(img, v_boxes, v_labels, v_scores):
    for i in range(len(v_boxes)):
